<template>
  <div>
    <SelectArea
      v-show="SelectAreaValue.isOpen"
      @selectadress="selectActions"
    ...
    <div v-show="contentIsOpen">
      ...
      <div class="details" style="background:#F5F5F5 ;width: 100%;">
        ...
        <div style="background: #fff;width: 100%;">
        	<div class="info" v-if="this.infoId==this.infoIdMember">
            <div class="flexs">
              <div class="info-left">联系地址</div>
              <div @click="selectAdress()" class="flexs flex">
                <div>
                  <p
                    v-show="!user.appnt.province&&!user.appnt.city"
                    style="color: #9E9E9E;text-align:right;"
                    class="text-hide info-size"
                  >请选择联系地址</p>
                  <p
                    v-show="user.appnt.province&&user.appnt.city&&user.appnt.county != null"
                    style="text-align:right;"
                    class="text-hide info-size"
                  >{{user.appnt.province|getProvinceText}}{{user.appnt.city|getCityText}}{{user.appnt.county}}</p>
                </div>
                <i class="fa fa-angle-right" style="color: #CDCDCD;"></i>
              </div>
            </div>
          </div>
          <div class="info" style="padding: 0 5% 2% 5%;" v-if="this.infoId!=this.infoIdMember">
            <div class="process-li">
              <div style="width:31%;">联系地址</div>
              <div style="display: flex;flex-direction: column;align-items: flex-start;height: auto;width:65%;">
              ...
              <p style="display:flex;align-items: center;line-height: 2;">
                <input type="checkbox" style="height:auto;" v-model="user.appnt.state" @change="changeSelfSyncAddress(user.appnt)"/>
                <span style="font-size: 14px;">与本人地址一致</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="info">
            <div class="flexs">
              ...
              <input
                ...
                type="text"
                placeholder="请输入详细地址"
                v-model="user.appnt.postalAddress"
              />
            </div>
          </div>
          <div class="info">
          ...
          <div class="info">
            <div class="flexs">
              ...
              <div class="flexs flex">
                <input class="info-size" type placeholder="请输入电子邮箱" v-model="user.appnt.email" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="footer">
          <mt-button
            style="width: 40%;background: #fff;color: #FEBA00;border-radius:5x;border: 1px solid #FEBA00;margin-right: 30px;margin-top: 10px;height: 44px;"
            @click="next()"
          >保存</mt-button>
          <mt-button
            style="width: 40%;background: #FEBA00;color: white;border-radius: 5px;height: 44px;margin-top: 10px;"
            @click="cancle()"
          >取消</mt-button>
        </div>
        <p>
...
export default {
  ...
  methods: {
    ...
    oneSelfInfo() {
      ...
      utils.http.postFast("BusinessService", requ, body => {
        if (body.data.newCustomerOne.resultCode == "0") {
          let contactAddress = body.data.memberLnfo.contactAddress;
          if (
            contactAddress != undefined &&
            contactAddress != null &&
            contactAddress.length > 0
          ) {
            this.oneSelfAddress = contactAddress;
            console.log("本人地址 : ", this.oneSelfAddress[0]);
          }
        }
      });
    },
    changeSelfSyncAddress(item){
    	if(item.state){
    		this.$nextTick(() => {
					item.province = this.oneSelfAddress[0].province;
		      item.city = this.oneSelfAddress[0].city;
		      item.county = this.oneSelfAddress[0].county;
		      item.street = this.oneSelfAddress[0].town;
		      item.community = this.oneSelfAddress[0].village;
		      item.postalAddress = this.oneSelfAddress[0].address;
		      item.zipCode = this.oneSelfAddress[0].zipNo;
          item.email = this.oneSelfAddress[0].email;
				})
    	}else{
    		this.$nextTick(() => {
					item.province = ''
    ...
    next() {
      ...
      let str1 =
        this.user.appnt.postalAddress +
        this.user.appnt.community +
        this.user.appnt.street;
      ...
      if (this.user.appnt.province != "") {
        ...
        this.user.appnt.province == "" ||
        this.user.appnt.province == undefined ||
        this.user.appnt.province == "undefined"
      ) {
        province = "";
      }
      if (this.user.appnt.city != "") {
        ...
        this.user.appnt.city == "" ||
        this.user.appnt.city == undefined ||
        this.user.appnt.city == "undefined"
      ) {
        city = "";
      }
      if (
        ...
        this.user.appnt.county == undefined ||
        this.user.appnt.county == "undefined"
      ) {
        this.user.appnt.county = "";
      }
      ...
      if (AlertText != "") {
        ...
        if (
          ...
          this.user.appnt.street == "" &&
          this.user.appnt.community == "" &&
          this.user.appnt.email == "" &&
          this.user.appnt.zipCode == ""
        ) {
          this.go(
            "/customer/customerInfo?infoId=" +
              this.infoId +
