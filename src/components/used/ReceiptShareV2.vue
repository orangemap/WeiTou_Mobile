<template>
	<div class="margin-top" style="font-size:12px">
		...
		<div v-show="iswechar" id="maincontent" style="overflow-y: hidden;" class="manage-main2-contents">
	...
	export default {
		...
		data() {
			return {
				...
				iswechar:false,
				imgUrl:'static/img/common/icon_in_wechar.png'
			}
		},
		mounted() {
			if(this.isWeiXin()){
	      		...
	      		this.iswechar = false
	      		let testdata = "张鑫$140724199403250073$12$M610136900599188$610003699788$-1$-1$-1"
				console.log(window.Base64.encode(testdata).replace(/\//g, '*'),"测试数据")
				this.data = window.Base64.decode(this.$route.params.args).replace(/\//g, '*').split("$")
				if(this.data.length!=8){
					alert_n22.alert('提示','参数非法,请确认链接是否已<br><br>全部复制,如有问题请联系管理员!')
				}
				console.log(this.data)
				console.log(typeof(this.data))
				//获取路由数据
				this.policy.appntName=this.data[0]
				this.policy.idNo=this.data[1]
				this.policy.idType=this.data[2]
				this.policy.policyCode=this.data[3]
				this.policy.applyCode=this.data[4]
				this.policy.isReceipt=this.data[5]
				this.policy.result=this.data[6]
				this.policy.resultmsg=this.data[7]
						...
						let requestParam = {
					 	...
					 	utils.http.postFast('SHORTLINKIMAGE',requestParam,(body)=>{
					   		...
					   		if(policy.resultCode == 0){
					   			this.isDown = true
					   			this.getPolicyUrl()
					     		alert_n22.alert('提示', policy.resultMessage + ',请关闭页面！')
								return
							}else{
