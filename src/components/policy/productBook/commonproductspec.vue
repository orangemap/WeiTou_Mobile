            ...
            <div id="firstTab">
                ...
                <div class="content">
                    <div v-for="items in newproductContent">
                        <div v-for="(item,index) in items.data">
                            <img :src="item.fileurl" style="display: block;width: 100%;">
                        </div>
                    </div>
                </div>
            </div>
...
export default{
    ...
    mounted(){
	    ...
	    if(CONFIG_proposalPlan==null||CONFIG_proposalPlan==''){
	    	...
	    	utils.n22_http.getConfig(0,(data)=>{
	    		...
	    		this.shuomingshuCallback(this.haveInstructionsCode)
	    	})
	    }else{
	    	//正常将缓存中的数据复制到相应节点
    ...
    methods:{
        //说明书过滤
        shuomingshuCallback(productCodes){
            let productContent = []
            for(let i in this.proposalPlans.proposalPlan.item){
                for(let j in productCodes){
                    if(productCodes[j] == this.proposalPlans.proposalPlan.item[i].productCode){
        ...
        jumpToBack(){
            if(this.isAgree){
                utils.cache.set('isReadInstructions','1')
        ...
        back(){
            utils.cache.set('isReadInstructions','0')
            if(!this.isShare){
                this.$router.push({
                	path: '/policy/ElectronicsPolicyPage.vue',
                	name: 'electronics_policy_page',
