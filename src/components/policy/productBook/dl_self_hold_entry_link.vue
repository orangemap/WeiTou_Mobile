<template>
	<div>
		...
		<div class="manage-contents">
			...
			<div id="DLSelfHoldLinkCtrl1" class="content clear">
				<div>
					<h1>自保件投保声明确认书（投连险）</h1>
					<p class="level">
						1、我是光大永明人寿保险有限公司大连分公司销售人员，执业证号是
						<input type="text" style="border-bottom: 1px solid black" v-model="DLSelfHoldLinkInfo.agentQuafNo">
		<!--                （内勤人员为身份证号码-->
		<!--                <input type="text" style="border-bottom: 1px solid black" readonly v-model="DLSelfHoldLinkInfo.agentIDCardNo">-->
		<!--                ）-->
		                ，现对我自己的签单过程进行全程记录。
					</p>
					<p class="level">
						2.我投保的是公司
						<!--<input type="text" style="border-bottom: 1px solid black" readonly v-model="DLSelfHoldLinkInfo.productName">，-->
						<span style="border-bottom: 1px black solid;width:30%;"s>{{DLSelfHoldLinkInfo.productName}}</span>
						产品，投保人和被保险人均是我本人，每年缴费
						<input type="text" style="border-bottom: 1px solid black;width:30%;" readonly v-model="DLSelfHoldLinkInfo.payment">
						元，缴费期是
						<input type="text" style="border-bottom: 1px solid black;width:30%;" readonly v-model="DLSelfHoldLinkInfo.paymentPeriod">
						年，保险期限是
						<input type="text" style="border-bottom: 1px solid black;width:30%;" readonly v-model="DLSelfHoldLinkInfo.periodYear">
						年（或至被保险人
						<input type="text" style="border-bottom: 1px solid black;width:30%;" readonly v-model="DLSelfHoldLinkInfo.periodTo">
						周岁），我已清楚知悉这份保单的犹豫期是从收到保险合同起
						<input type="text" style="border-bottom: 1px solid black;width:30%;" readonly v-model="DLSelfHoldLinkInfo.hesitationPeriod">
						天，并已清楚了解犹豫期内外退保的区别。
					</p>
					<p class="level">3.我清楚了解本产品的保险责任是
						<u style="letter-spacing: 1px;line-height: 24px;">
							{{DLSelfHoldLinkInfo.insuranceLiability}}
						</u>
						免除责任是
						<u style="letter-spacing: 1px;line-height: 24px;">
							{{DLSelfHoldLinkInfo.exemptionFromLiability}}
						</u>
						<!-- <input type="text" style="border-bottom: 1px solid black" readonly/> -->
						<!-- <input type="text" style="border-bottom: 1px solid black" v-model="DLSelfHoldLinkInfo.insuranceLiability">；
		                <input type="text" style="border-bottom: 1px solid black" v-model="DLSelfHoldLinkInfo.exemptionFromLiability">。 -->
						<u>相关详细内容我已全部清楚了解。</u>
					</p>
					...
					<p class="level">
						7.我清楚了解
						<u style="letter-spacing: 1px;">{{DLSelfHoldLinkInfo.productName}}</u>
						<!-- <input type="text" style="border-bottom: 1px solid black" v-model="DLSelfHoldLinkInfo.productName"> -->
						产品的投资回报率是不确定的，实际投资收益可能会出现亏损；我已清楚知悉宣传材料上的利益演示是基于保险公司的精算假设，不代表未来的实际收益；我已清楚知悉了保单初始费用、保单管理费、风险保障费等费用扣除项目及扣除的比例（或金额）。
					</p>
					<p class="level">
						8.我清楚了解知悉该产品各账户的投资方向、相关投资风险及各账户间转换规定。
					</p>
					<p class="level">
						9.我清楚了解该产品的赎回相关规定。
					</p>
			...
			<div style="margin:5% 0 0 0;line-height: 20px;">
			    <input type="checkbox" v-model="isAgree">&nbsp;&nbsp;本人{{DLSelfHoldLinkInfo.appntName}}已认真阅读投保特别提示，且全面知晓理解相关内容，并予以确认。
			</div>
		</div>
	...
	export default{
            ...
            if(documentInfo!=null && documentInfo!=undefined && documentInfo.length>0){
                for(var i=0;i<documentInfo.length;i++){
                    if(documentInfo[i].name == this.saveName){
                        this.DLSelfHoldLinkInfo = documentInfo[i].content
                    }
                }
            }else{
        ...
        mounted(){
            ...
            for(var i=0;i<products.length;i++){
            	...
            	console.log("险种名称=====",this.DLSelfHoldLinkInfo.productName)
            	if(products[i].isMaster=="1"){
            		//缴费期
            		this.DLSelfHoldLinkInfo.paymentPeriod=this.getPayFreq(products[i].payYear)	
            		//保险期限
            		this.DLSelfHoldLinkInfo.periodYear=this.getInsure(products[i].insureYear)
            	}
            }
            ...
            this.DLSelfHoldLinkInfo.productName=productNameList.substring(0,productNameList.length-1)
            //缴费*元
            this.DLSelfHoldLinkInfo.payment=premSum
            //保险期限至被保险人多少岁periodTo
            // this.DLSelfHoldLinkInfo.periodTo=
            //保险责任免除责任
            this.getDlSelfHoldEntryLink()
        },
	    methods:{
	    	jumpToBack(){
	    		// if(this.isShare){
	    	    		...
	    	    		this.$router.push({
	    	    				...
	    	    				params: {
	    	    						...
	    	    						sendInfo:{
	    	    							name:"2050901_06",
	    	    							isRed:"0",  
	    	    							content:this.DLSelfHoldLinkInfo
	    	    						}
	    	    ...
	    	    utils.http.post('MOHQPRODUCTMSG',requ).then(response => {
	    	        ...
	    	        if(response.data.header.responseCode == "0"){
    	        		let insuranceList =response.data.response.policy.proposalCoverageItemsList;
	    	           	...
	    	           	this.DLSelfHoldLinkInfo.insuranceLiability = insuranceLiability_main+" "+insuranceLiability_extra
	    	           	this.DLSelfHoldLinkInfo.exemptionFromLiability = exemptionFromLiability_main+" "+exemptionFromLiability_extra
	    	        }
	    	    }).catch(error => {
	    	...
	    	checkall(){
	    		var reg=/^\d{10}$/;
				if(this.DLSelfHoldLinkInfo.agentQuafNo.replace(/(^s*)|(s*$)/g, "").length == 0) {
					this.$refs.alert.showMOdal("请填写代理人执业证号")
					return false
				}else if(!reg.test(this.DLSelfHoldLinkInfo.agentQuafNo)){
