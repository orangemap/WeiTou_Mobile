<template>
  <div :class="cls">
    ...
    <div :class="`${prefixCls}-actions ${prefixCls}-actions-left`"ref="leftBtns">
      <div v-for="(btn, i) in left" 
        :key="i" 
        :class="`${prefixCls}-btn`"
        :style="btn.style"
        role="button"
        @click="(e) => onBtnClick(e, btn ,btn.data)">
        <div :class="`${prefixCls}-btn-text`">{{ btn.text || 'Click' }}</div>
      </div>
    </div>
    <div :class="`${prefixCls}-actions ${prefixCls}-actions-right`" ref="rightBtns">
      <div v-for="(btn, i) in right" 
        :key="i" 
        :class="`${prefixCls}-btn`"
        :style="btn.style"
        role="button"
        @click="(e) => onBtnClick(e, btn,btn.index ,btn.data)">
        <!--<div :class="`${prefixCls}-btn-text`">{{ btn.text || 'Click' }}</div>-->
        <img :class="`${prefixCls}-btn-text`" style="width: 2rem;height: 2rem;" :src=btn.src alt="" />
      </div>
    </div>
    <v-touch
...
export default {
  ...
  methods: {
    ...
    onBtnClick (ev, btn , index ,data) {
      const onPress = btn.onPress;
      if (onPress) {
        onPress(ev, index ,data);
      }
      if (this.autoClose) {
        this.close();
      }
