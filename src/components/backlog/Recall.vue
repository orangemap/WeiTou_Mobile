			...
			<div class="main-contents">
				<div class="details">
					...
					<div style="margin-bottom: 5%;">
						...
						<textarea v-model="calCellCause" rows="4" placeholder="请输入变更原因" style="width:100%;padding: 5%;box-sizing: border-box;font-size: 14px;"></textarea>
					</div>
					<div>
						<p>客户声明：</p>
					...
					<div style="text-align: center;margin: 13% 0px 15%;">
						...
						<img v-show="appntSign==''" id="xss_20" :src="wrting" @click="faceTest">
						<img v-show="appntSign!=''" id="xss_20" :src="appntSign" @click="faceTest">
					</div>
...
<script>
	...
	export default {
		...
		beforeMount() {
			...
			this.shareLink = protocol + "://" + host + "/com.ifp.ipartner/proposalShare/index.html#/recallShare/" + this.orderId + "/" + this.applyCode + "/-1/-1/-1"
		},
		computed: {
			caShow() {
					...
					if(idCards != undefined && idCards.length > 0) {
						for(let i = 0; i < idCards.length; i++) {
							if(idCards[i].imageName == "24CJ") {
								this.appntSign = idCards[i].reaPath
								let image = document.getElementById('xss_20')
								this.caValue.image = this.getBase64Image(image)
							}
						}
					}
				})
