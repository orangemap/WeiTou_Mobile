<template>
	<div class='upload-img'>
		...
		<div class='upload-image-div'>
			...
			<p class='text-center'>申请人： {{rptorname}} 先生/女士</p>
...
<script>
	...
	export default {
		...
		computed:{
			...mapState(['VuxPolicyType','policyData','policyCode','newPolicyType','policyState','fileType','policyId','newPolicyCode','policy_2_imgList'])
		},store,
		mounted() {
			...
			console.log('赔案号----->>>',this.newPolicyCode)
			console.log('出险类型----->>>',this.newPolicyType)
			console.log('影像类型----->>>',this.$route.params.id)
		...
		methods: {
			...mapMutations(['SetnewPolicyType','SetPolicyState','Setpolicy_2_imgList']),
			Aleart(message) {
			...
			select_img(){
				...
				let than = this
				let url = this.imagePaths.length == 0?'':this.imagePaths
				MAPlugin.hybridCallAction('hybrid://MAImageSelect:401/invokeImgSel?'+url, (callbackSucc)=>{
					console.log('初始化数据')
					this.imagePaths = this.imagePaths.concat(callbackSucc.result.split('|'))
				...
				alert_n22.confirm('提示', '确定删除这张照片吗?', '确定', '取消', (flag) => {
					...
					utils.http.postFast('CusbusinessService',requ,(body,header) => {
						...
						if(body.data.code == 0){
							...
							this.policy_2_imgList.forEach( (item,index) => {
								if(item == id){
									this.policy_2_imgList.splice(index,1)
								}
							})
							this.Setpolicy_2_imgList(this.policy_2_imgList)
							this.Aleart('删除成功')
						}
					})
				})
			...
			backUrl() {
				if(this.imgList.length>0){
					console.log('返回时,补传影像idList',this.policy_2_imgList)
					this.Setpolicy_2_imgList(this.policy_2_imgList)
					let arr = []
					...
					utils.http.postFast('CusbusinessService',requ,(body,header) => {
						if(body.data.code == 0){
							this.$router.push('/settlementUploadListAdd/'+this.policyId)
						}
					})
				}else{
					this.$router.push('/settlementUploadListAdd/'+this.policyId)
			...
			addAppImageAndUpload(imageUrl) {
				...
				let requ =  {
					data:{
						fileBase64:"data:image/jpeg;base64," + imageUrl,
						claimsId:this.newPolicyCode,/*报案号*/
						claimsType:this.newPolicyType,/*出险类型*/
						fileType:this.$route.params.id,/*影像类型*/
						imageType:"jpg",
				...
				utils.http.postFast('CusbusinessService',requ,(body,header) => {
					...
					if(body.data.code == 0){
						...
						this.policy_2_imgList.push(body.data.fileId)
						this.imgList.push(items)
						this.imgList.forEach( item => {
							this.policy_2_imgList.forEach( tpl => {
								if(item.fileId == tpl){
									item.del = true
								}
							})
			...
			getInitData(){
				let requ = {
					"data": {
						"claimNo":this.policyId,//'3020e20eb5ed4139b36eaf6f2d00593a'
						"claimType":"1",
						"step":"3"
					},
					modelName: "claimService",
				...
				utils.http.postFast('CusbusinessService',requ,(body,header) => {
					...
					console.log('补传的影像id------>>>',this.policy_2_imgList)
					if(body.data.code == 0){
						this.rptorname = body.data.claim.applicants[0].accname
						this.imgList = []
						let arr = body.data.claim.imageFiles
						if(arr.length>0){
							for(let i=0;i<arr.length;i++){
								arr[i].key = i+1
								if(arr[i].fileType == this.$route.params.id){
									let newObject = {
										key:arr[i].key,
										fileId:arr[i].id,
										url:arr[i].url
