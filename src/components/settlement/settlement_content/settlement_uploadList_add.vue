<template>
	<div>
		...
		<div class='set-upload set-upload-list set-upload-new'>
...
<script>
	...
	export default {
		...
		computed:{
			...mapState(['fileType','policyId','policyData','newPolicyCode','newPolicyType','policy_2_imgList'])
		},store,
		mounted() {
			let htmlDOM = document.getElementsByTagName('html')[0]
			htmlDOM.style.fontSize = this.screenWidth/20+'px'
			console.log('赔案号',this.$route.params.id)
		...
		methods: {
			...mapMutations(['SetpolicyId','SetpolicyData','SetnewPolicyCode','SetnewPolicyType','Setpolicy_2_imgList']),
			Aleart(message) {
				...
				utils.http.postFast('CusbusinessService',requ,(body,header) => {
					...
					if(body.data.claim.claimId){
						...
						this.policy_2_imgList.forEach((item,index) => {
							let id = index === (this.policy_2_imgList.length -1)?item : item+','
							this.fileImgList += id
						})
					...
					/*取出险类型*/
					// newPolicyType
					let typeArr = body.data.claim.accidentInfo.accidentMode
					console.log(typeArr)
					let typeStr = ''
					typeArr.forEach( item=> {
						if(item.isselected == '1'){
							typeStr+=item.reasoncode+','
						}
					})
					typeStr = typeStr.substring(0,typeStr.length-1)
					this.SetnewPolicyType(typeStr)
			...
			goToImg(id){
				this.$router.push('/settlementUploadAdd/'+id)
			},
			UploadSbm() {
				alert_n22.confirm('提示', '本次补传'+this.policy_2_imgList.length+'张照片', '确定', '取消', (flag) => {
					let requ = {
						data: {
							"uuid":this.uuid,
...
.set-upload-list>div:last-child{
	margin-bottom:20px;
}
.uploader-add-btn{
	text-align: center;
