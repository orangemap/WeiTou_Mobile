<template>
	<div class='data-affirm data-affirm-4'>
		<div class='data-affirm-container' style='width: 100%;padding:0'>
			...
			<div style='margin-bottom:5.5rem;min-height: 290px;' class='rows'>
				...
				<div style='display: flex;justify-content: flex-end;'>
					...
					<p v-for='item in applicants' style='width:80px;height:60px;'>
						<span>申请人签字</span>
						<!--<img :src="item.writing2" style='width:60px;height:60px;'>-->
						<a :href="href.signToken">
							<img style='width:60px;height:60px;' :src="item.writing2">
						</a>
					</p>
				</div>
			</div>
	...
	export default {
		...
		data() {
			return {
				...
				result:'',
				resultmsg:'',
				policyCode:this.$route.params.claimsId.split('_')[0],
				...
				agentAddressCode:this.$route.params.claimsId.split('_')[1],//代理人地址
				writing1:'',//代办人签字影像
		...
		computed: {
			...mapState(['policyState','newPolicyType'])
		},store,
		mounted() {
			...
			this.getInitData()
			this.fileImgList = this.resetFileImgList(this.$route.params.fileId)
			console.log('待上传的影像id----->>>',this.fileImgList)
			console.log(this.agentAddressCode)
			$('.data-affirm-header').css('top',$('.v-header').outerHeight(true)+'px')
			$('.data-affirm').css('height',this.media.container(this.screenHeight,0)+'px').css('overflow','hidden')
			$('.data-affirm-container').height($('.data-affirm').outerHeight(true)-40+'px').css('overflow-y','scroll').css('margin-top',$('.data-affirm-header').outerHeight(true)+'px')

			this.writing1 = this.writing/*初始化代办人签字图片*/
			this.writing2 = this.writing/*初始化申请人签字图片*/
		},
		methods: {
			...mapMutations(['SetnewPolicyType']),
			resetFileImgList(str){
				//第一步:将字符串按逗号分隔为数组,单个元素大于32位的进行逗号添加操作
				let newFileIds = []
				let fileIds = str.split(',')
				for(let i=0;i<fileIds.length;i++){
					if(fileIds[i].length>32){
						let tag = fileIds[i].length/32
						for(let j=0;j<tag;j++){
							newFileIds.push(fileIds[i].substring(j*32,32*(j+1)))
						}
					}else{
				...
				for(let i=0;i<newFileIds.length;i++){
					if(uniqArr.indexOf(newFileIds[i]) == -1){
			            uniqArr.push(newFileIds[i]);
			        }
				}
				//第三步重组FileImgList参数
				let newstr = ''
			...
			faceTest(id,type) {/*进行人脸识别*/
				...
				if(id == 1){
					...
					MAPlugin.hybridCallAction('hybrid://MAFaceIdentify:401/invokeFaceIdentify?', (callbackSucc) => {//插件调用需真机操作
						this.ImgGet('face_appl','sign_appl',callbackSucc)/*调用接口，上传图片，区分类型*/
					}, (callbackFail) => {
			...
			Next() {
				if(this.applicantSignFileId != ''){
					let requ = {
						data: {
							applicantSignFileId:this.applicantSignFileId,//申请人签字文件返回id
							linkManFileId:this.$route.params.claimsId.split('_')[2],//代办人签字文件id
			...
			getInitData() {
				...
				utils.http.postFast('CusbusinessService',requ,(body,header) => {
					...
					if(body.data.code == 0){
						...
						let signToken = this.applicants[0].bnfname+"-"+
										this.applicants[0].bnfidno+"-"+
										this.applicants[0].bnfidtype+"-"+
										this.applicants[0].orders+"-"+
										this.$route.params.claimsId+"-"+
										this.$route.params.fileId
						let host = window.globalConfig.rootUrl.split('/')[2]
						let protocol = window.globalConfig.rootUrl.split(':')[0]
						this.href.signToken = protocol + "://" + host + "/com.ifp.ipartner/claims/faceid/" + window.Base64.encode(signToken).replace(/\//g,'*')
						console.log('申请人姓名===>',this.applicants[0].bnfname)
						console.log('申请人证件号===>',this.applicants[0].bnfidno)
						console.log('申请人证件类型===>',this.applicants[0].bnfidtype)
						console.log('申请人序号===>',this.applicants[0].orders)
						console.log(this.href.signToken)
						//根据识别结果处理后续流程
						if(this.result!=''&&this.result!=0){
							this.$refs.showalert.showMOdal(this.resultmsg)
						}else if(this.result!=''&&this.result==0){
							if(this.applicants[0].bnfidtype == '11'){
								if(this.image_best!=undefined && this.image_best!=''){
									this.appliFaceId = this.image_best
									this.openCA(2)
								}else{
									this.showModals("无人脸识别信息,申请无法继续!")
								}
							}else{
								this.appliFaceId = ''
								this.openCA(2)
