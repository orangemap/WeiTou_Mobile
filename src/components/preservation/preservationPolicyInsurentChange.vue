			...
			<div>
				...
				<div class="insurentContent">
					<div class="info cerinfo">
						<div>
							<span style="margin-right: 10%;">{{insurent.insuName}}</span>
							<span style="margin-right: 10%;">{{insurent.insuSex == "0"?"男":"女"}}</span>
							<!--<span>{{}}</span>-->
						</div>
						<div>
							<span>证件类型：{{idTypeText(insurent.insuIDType)}}</span>
						</div>
						<div>
							<span>证件号码：{{insurent.insuIDNo}}</span>
						</div>
						<div>
							<span>证件有效起期：{{insurent.iDStartDate}}</span>
						</div>
						<div style="margin-bottom: 10px;">
							<span>证件有效止期：{{insurent.insuIDExpDate}}</span>
						</div>
					</div>
...
//					this.holder.endDate=this.cardScanningValue.cusItem.valid_date
					...
					this.age=utils.valid.getAge(this.cardScanningValue.cusItem.birthday)
					...
					let requs = {
						data: {
							...
							"custNo":this.insurent.insuCustNo
						},
...
//			this.holder.endDate=this.perData.endDate==''?'请选择':this.perData.endDate
			...
			let aa=this.policyData.insurent
			let queryParams = this.$route.query;
			if(queryParams){
				if(queryParams.id){
			        this.insurentId = queryParams.id;
			      }
				if(queryParams.nums){
			...
			for(let i=0;i<aa.length;i++){
				if(this.insurentId==aa[i].id){
					this.insurent=aa[i]
				}
			}
			this.age=utils.valid.getAge(this.insurent.insuredBirthday)
			//当前页面适配
...
//				console.log()
			...
			confirm(){
				...
				if(this.holder.cerCard!=this.insurent.insuIDNo){
					showModals='变更人证件号码不一致。'
				}
				if(this.ischecked==false){
					if(this.holder.endDate == '' || this.holder.endDate == ''){
...
//					console.log(newDate)
				...
				if(this.holder.startDate == '' || this.holder.startDate == '') {
				...
				if(this.notationCulvert == '' || this.notationCulvert == ''){
				...
				if(showModals != ''){
	               this.showModals(showModals)
	            	...
	            	if(this.insurent.insuIDType=='11'){
...
//		            	this.perData.endDate=this.holder.endDate
		            	...
		            	this.SetpolicyCode(this.perData)
		            					this.go('/preservation/preservationPolicyChangeAffirm?tpl='+this.bb+'&id='+this.insurentId)
	            	}else{
