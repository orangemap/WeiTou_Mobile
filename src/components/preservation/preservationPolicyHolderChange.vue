				...
				<div>
					...
					<div class="insurentContent">
						<div class="info cerinfo">
							<div>
								<span style="margin-right: 10%;">{{policyData.holder.appntName}}</span>
								<span style="margin-right: 10%;">{{policyData.holder.appntSex == "0"?"男":"女"}}</span>
								<span>{{policyData.holder.appntBirthday}}</span>
							</div>
							<div>
								<span>证件类型：{{idTypeText(policyData.holder.appntIDType)}}</span>
							</div>
							<div>
								<span>证件号码：{{policyData.holder.appntIDNo}}</span>
							</div>
							<div>
								<span>证件有效起期：{{policyData.holder.IDStartDate}}</span>
							</div>
							<div style="margin-bottom: 10px;">
								<span>证件有效止期：{{policyData.holder.appntIDExpDate}}</span>
							</div>
						</div>
						<div class="info delInfo">
							<div class="flex">
								...
								<div class="right">
									<!--<input type="text" placeholder="请输入" style="text-align: end;" v-model="holder.cerCard" :values="holder.cerCard" maxlength="18"/>-->
						...
						<div class="info delInfo">
							<div class="flex">
								...
								<div class="right">
									<!--<date-picker v-model="holder.startDate" style="font-size: 1.2rem;" :values="holder.startDate"></date-picker>-->
...
//			console.log(this.policyData)
			this.age = utils.valid.getAge(this.policyData.holder.appntBirthday)
...
//					console.log(this.holder.endDate)
				...
				if(showModals != '') {
					...
					if(this.policyData.holder.appntIDType == '11') {
						...
						this.SetpolicyCode(this.perData)
						this.go('/preservation/preservationPolicyChangeAffirm?tpl=' + this.aa)
					} else {
