								...
								<div style="margin:auto 4px;margin-bottom:5.5rem;">
									...
									<div v-if="change=='change' || whereFrom != 'pList'">
										...
										<div style="width:5rem;height:5rem;float:right;padding-right:3rem;">
											<a :href="href.signToken">
												<img style="width:5rem;height:5rem;" :class="{rotate:isRotate}" :src="writing">
											</a>
										</div>
									</div>
...
//				}
				if(queryParams.serverId) {
					...
					//如果存在此节点需将conservationValueId赋值到相应保全项目id上
					if(queryParams.conservationValueId){
						if(this.serverId == 'PC'){//保单交费
							this.conservationPolicyPayId = queryParams.conservationValueId
						}else if(this.serverId == 'IP'){//保险费追加
							this.conservationFeeAddPayId = queryParams.conservationValueId
						}else if(this.serverId == 'AR'){//账户价值提取
							this.conservationAccountvalueId = queryParams.conservationValueId
						}else if(this.serverId == 'LR'){//保单补发
							this.conservationPolicyresendId = queryParams.conservationValueId
						}else if(this.serverId == 'WT'){//犹豫期退保 此处为特殊处理,拼接签字链接时处理
							this.conservationRetreatsPayId = queryParams.conservationValueId
						}else if(this.serverId == 'CT'){//退保 此处为特殊处理,拼接签字链接时处理
							this.conservationRetreatsPayId = queryParams.conservationValueId
						}
					}
				}
				if(queryParams.isHesitationTime){
					...
					if(this.isHesitationTime == undefined || this.isHesitationTime == 'undefined'){
						this.isHesitationTime = 'R'
					}
				}else{
					this.isHesitationTime = 'R'
				}
				...
				if(queryParams.image_best){//
					this.image_best=queryParams.image_best
				}
				if(queryParams.result){//
					this.result=queryParams.result
				}
				if(queryParams.resultmsg){//
					this.resultmsg=queryParams.resultmsg
					this.resultmsg = "人脸识别比对不成功，无法线上申请该业务。"
				}
				if(queryParams.connoTpp){//
					this.connoTpp=queryParams.connoTpp
				}else{
					this.connoTpp = 'undefined'
				}
				if(queryParams.whereFrom){//
					this.whereFrom=queryParams.whereFrom
					if(this.whereFrom == undefined || this.whereFrom == 'undefined'){
						this.whereFrom = 'R'
					}
				}else{
					this.whereFrom = 'R'
				...
				if(queryParams.change){//
					...
					if(this.change == undefined || this.change == 'undefined'){
						this.change = 'R'
					}
				}else{
					this.change = 'R'
				...
				if(queryParams.t){//
			        ...
			        let onDay = 86399999;
			        console.log(onDay)
			        let currentTimestamp = (new Date(Y+M+D+" 00:00:00")).getTime()
	        		console.log(currentTimestamp)
			        let nextTimestamp = (new Date(Y+M+D+" 00:00:00")).getTime()+onDay
	        		console.log(nextTimestamp)
	        		if(queryParams.t<currentTimestamp||queryParams.t>nextTimestamp){
						alert_n22.confirm('提示', '当前链接已失效,请及时联系业务员!', '确定','', (flag) => {
						})
						$('.body-panel>.btns').css('display', 'none')
						$('.body-panel>.content').css('border-bottom', 'none')
	        		}
				}
		    }
			let date = new Date()
			...
			if(this.serverId == 'PC'){//保单交费
				...
				utils.http.postFast('CusbusinessService', requ, (body) => {
					if(body.data.code == '0') {
						...
						if(this.conservation.conno != undefined && this.conservation.conno != "" && this.whereFrom != 'pList'){
							alert_n22.confirm('提示', '当前链接已失效,请及时联系业务员!', '确定','', (flag) => {
							})
							$('.body-panel>.btns').css('display', 'none')
							$('.body-panel>.content').css('border-bottom', 'none')
						...
						//点击签字超链接触发后台获取Token进入旷世识别流程
						//姓名+证件号码+证件类型+保全id+账户价值id+保全项
						if(this.accountData.getidtype==undefined||this.accountData.getidtype == ''){
							this.accountData.getidtype = '11'
						}
						let signToken = this.accountData.name+"-"+
										this.conservation.holderidno+"-"+
										this.accountData.getidtype+"-"+
										this.conservationId+"-"+
										this.conservationPolicyPayId+'-'+
										this.serverId+'-'+
										this.connoTpp+'-'+
										this.whereFrom+'-'+
										this.change+'-'+
										this.isHesitationTime
						let host = window.globalConfig.rootUrl.split('/')[2]
						let protocol = window.globalConfig.rootUrl.split(':')[0]
						this.href.signToken = protocol + "://" + host + "/com.ifp.ipartner/conservation/faceid/" + window.Base64.encode(signToken).replace(/\//g,'*')
						console.log('申请人姓名===>',this.accountData.name)
						console.log('申请人证件号===>',this.conservation.holderidno)
						console.log('申请人证件类型===>',this.accountData.getidtype)
						console.log('申请人保全id===>',this.conservationId)
						console.log('申请人保全项id===>',this.conservationPolicyPayId)
						console.log('申请人服务id===>',this.serverId)
						console.log('申请人connoTpp===>',this.connoTpp)
						console.log('申请人whereFrom===>',this.whereFrom)
						console.log('申请人change===>',this.change)
						console.log('申请人isHesitationTime===>',this.isHesitationTime)
						console.log('申请人签字地址===>',this.href.signToken)
						//根据识别结果处理后续流程
						if(this.result!=''&&this.result!=0){
							this.showModals(this.resultmsg)
						}else if(this.result!=''&&this.result==0){
							if(this.image_best!=undefined && this.image_best!=''){
								this.uploadFaceImg(this.image_best)
							}else{
								this.showModals("无人脸识别信息,保全无法继续!")
							}
						}
					} else {
...
//						this.accountData.fee=Number(this.accountData.fee)+Number(this.accountData.ipfymoney)
						...
						console.log('申请人证件类型===>',this.accountData.getidtype)
						console.log('申请人保全id===>',this.conservationId)
						console.log('申请人保全项id===>',this.conservationFeeAddPayId)
						console.log('申请人服务id===>',this.serverId)
						console.log('申请人connoTpp===>',this.connoTpp)
						console.log('申请人whereFrom===>',this.whereFrom)
						console.log('申请人change===>',this.change)
				...
				let connoTpp = this.connoTpp
				if(connoTpp==undefined || connoTpp=='undefined' ){
					connoTpp = ""
				}
				let requ = {
					data: {
				...
				utils.http.postFast('CusbusinessService', requ, (body) => {
					if(body.data.code == '0') {
						...
						let signToken = this.accountData.name+"-"+
										...
										this.accountData.getidtype+"-"+
										this.conservationId+"-"+
										this.conservationAccountvalueId+'-'+
										this.serverId+'-'+
										this.connoTpp+'-'+
										this.whereFrom+'-'+
										this.change+'-'+
						...
						console.log('申请人证件类型===>',this.accountData.getidtype)
						console.log('申请人保全id===>',this.conservationId)
						console.log('申请人保全项id===>',this.conservationAccountvalueId)
						console.log('申请人服务id===>',this.serverId)
						console.log('申请人connoTpp===>',this.connoTpp)
						console.log('申请人whereFrom===>',this.whereFrom)
						console.log('申请人change===>',this.change)
				...
				utils.http.postFast('CusbusinessService', requ, (body) => {
					if(body.data.code == '0') {
						...
						let signToken = this.accountData.name+"-"+
										...
										this.accountData.getidtype+"-"+
										this.conservationId+"-"+
										this.conservationPolicyresendId+'-'+
										this.serverId+'-'+
										this.connoTpp+'-'+
										this.whereFrom+'-'+
										this.change+'-'+
						...
						console.log('申请人证件类型===>',this.accountData.getidtype)
						console.log('申请人保全id===>',this.conservationId)
						console.log('申请人保全项id===>',this.conservationPolicyresendId)
						console.log('申请人服务id===>',this.serverId)
						console.log('申请人connoTpp===>',this.connoTpp)
						console.log('申请人whereFrom===>',this.whereFrom)
						console.log('申请人change===>',this.change)
				...
				utils.http.postFast('CusbusinessService', requ, (body) => {
					if(body.data.code == '0') {
						...
						let signToken = this.accountData.name+"-"+
										...
										this.accountData.getidtype+"-"+
										this.conservationId+"-"+
										this.conservationRetreatsPayId+'-'+
										this.serverId+'-'+
										this.connoTpp+'-'+
										this.whereFrom+'-'+
										this.change+'-'+
						...
						console.log('申请人证件类型===>',this.accountData.getidtype)
						console.log('申请人保全id===>',this.conservationId)
						console.log('申请人保全项id===>',this.conservationRetreatsPayId)
						console.log('申请人服务id===>',this.serverId)
						console.log('申请人connoTpp===>',this.connoTpp)
						console.log('申请人whereFrom===>',this.whereFrom)
						console.log('申请人change===>',this.change)
		...
		methods: {
			uploadFaceImg(face_image){
				this.faceNum = face_image
				this.openCA();
			},
			sure() {
				//this.go('/home')
			...
			detailBack() {
				...
				if(this.serverId == 'PC') {
					...
					this.go('/preservation/preservationAccount?serverId='+this.serverId+'&conservationId='+this.conservationId+'&conservationAccountvalueId='+this.conservationAccountvalueId+'&num='+this.num)
					...
					this.go('/preservation/preservationSurrender?serverId='+this.serverId+'&isHesitationTime='+this.isHesitationTime+'&conservationId='+this.conservationId+'&conservationRetreatsPayId='+this.conservationRetreatsPayId+'&num='+this.num)
