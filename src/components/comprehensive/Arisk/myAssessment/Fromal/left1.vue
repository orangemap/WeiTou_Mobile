		...
		<ul class='ass-cnt-btm-list'>
			<li>
				<div class='flex'>
					...
					<div style='width:25%' class='flex'>
						<p :class="[Gractive1?'aaa':'green']">{{warning.perNPC.perNPC}}</p>
			...
			<li>
				<div class='flex'>
					<p>综开业绩</p>
					<div style='width:25%' class='flex'>
						<p :class="[Zkactive1?'aaa':'green']">{{warning.zongAchit.grptoagentNPC}}</p>
...
<script>
	...
	export default {
		...
		mounted(){
			...
			this.getMessage(this.agentCode,year,this.monthDate,'1','1')
		},
		computed: {
		...
		methods:{
			...
			BtmListShow(id){
				...
				this.List1.forEach( item => {
					...
					if(item.show) {
						item.url = './static/img/comprehensive/top.png'
					}else {
			...
			getMessage(agentCode,year,month,index,flag){
				...
				utils.http.postFast('ZHFORMALDETAILGX',requ,(body,header) => {
					...
					if(header.resultCode == 0){
						...
						console.log('baoji',data.keepAssessment.keepflag)
						...
						if(index == 1){
							...
							if(data.keepAssessment.warning.group.firstgrpCj == '0.00'){
							...
							/**********继续率（百分比）**********/
							console.log(data.keepAssessment.warning.continuationRate.preconrate)
...
getDataMessage(){
	let year = this.yearDate.toString()
	this.getMessage(this.agentCode,year,this.monthDate,'1','2')
}
},
watch:{
	// yearDate(tpl){
...
.Formleft-div-cnt>div:nth-child(3){
	width:27%;
}
.Formleft-div-cnt>div:nth-child(2){
...
.left-1 .ass-cnt-btm-list>li>ul{
	width:80%!important;
	margin: 0 auto!important;
}
.left-1 .ass-cnt-btm-list>li>ul>li{
	width:100%!important;
	height:30px;
}
.ass-cnt-btm-list>li>ul>li>p:last-child{
	width:50%;
	text-align: right;
}
.ass-cnt-btm-list>li>div>div>p:nth-child(1){
	text-align: right;
	flex:1;
}
